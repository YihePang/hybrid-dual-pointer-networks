...
def attention_layer(from_tensor,              
	...
	attention_mask: (optional) int32 Tensor of shape [batch_size, from_seq_length, to_seq_length]. 
		...
		set to -infinity for any positions in the mask that are 0, and will be unchanged for positions that are 1.
	...
	do_return_2d_tensor: bool. 
				...
				If False, theoutput will be of shape [batch_size, from_seq_length, num_attention_heads * size_per_head].
	...
	# `query_layer` = [B, N, F, H]
	# [batch_size,  num_attention_heads,   seq_length,  head_width]
	...
	if attention_mask is not None:     
		# `attention_mask`[batch_size, from_seq_length, to_seq_length] ===》 [B, 1, F, T]
	...
	attention_probs = tf.nn.softmax(attention_scores)     # softmax
	...
	# `context_layer` = [B, N, F, T] * [B, N, T, H] = [B, N, F, H]
...
def transformer_model(input_tensor,
	...
	"""
	Args:
		input_tensor: float Tensor of shape [batch_size, seq_length, hidden_size].   #input : self-att encoder
		...
		num_hidden_layers: int. Number of layers (blocks) in the Transformer.         #block number
		num_attention_heads: int. Number of attention heads in the Transformer.       #one block head number
  ...
  """
	...
	if input_width != hidden_size:
		raise ValueError("The width of the input tensor (%d) != hidden size (%d)" %  (input_width, hidden_size))   #抛出异常
...
class trans_encoder_config(object):        
	def __init__(self,
				 hidden_size=768,             
				 num_hidden_layers=12,        
				 num_attention_heads=12,      
...
class transformer_encoder(object):
	"""
	Args:
	  config: `BertConfig` instance.
	  is_training: bool. rue for training model, false for eval model. Controls
		whether dropout will be applied.
	  input_ids: int32 Tensor of shape                             
	  input_mask: (optional) int32 Tensor of shape                
	  token_type_ids: (optional) int32 Tensor of shape             
	...
	def __init__(self,
		...
		input_shape = get_shape_list(input_emb, expected_rank=3)
		batch_size = input_shape[0]                         
		seq_length = input_shape[1]                        
		...
		with tf.variable_scope(scope, default_name="transformer"):
			...
			with tf.variable_scope("encoder"):
				# input_mask = tf.sequence_mask(s_len, max_len, dtype=tf.int32, name='mask')
				# This converts a 2D mask of shape [batch_size, seq_length] to a 3D
				# mask of shape [batch_size, seq_length, seq_length] which is used
				# for the attention scores.
				attention_mask = create_attention_mask_from_input_mask(                  
					input_emb, input_mask)  
	...
	def get_pooled_output(self):
		return self.pooled_output    #[batch_size, hidden_size]
